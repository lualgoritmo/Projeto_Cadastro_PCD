          ...
          <?php
           foreach($ret as $usuario){
             ...
             <tr>
              ...
              <td>";
              $date = date_create($usuario -> dataMatricula);
              echo date_format($date, 'd/m/Y');
              echo "</td>
              <td>{$usuario -> sexoUsuario}</td>
              <td>{$usuario -> numeroTelefone}</td>
              <td{$usuario -> logradouro}</td>
              <td>{$usuario -> email}</td>
              <td>{$usuario -> estado}</td>
              <td>
                <button class='btn btn btn-warning'><a href='index.php?controle=controleUsuario&metodo=editarPCD&idPCD={$usuario->idUsuarioPCD}'>Visualizar</a></button>
              </td>
              <td>
                 <button class='btn btn btn-light'> <a href='index.php?controle=controleUsuario&metodo=excluirPCD&idPCD={$usuario->idUsuarioPCD}'>Excluir</a></button>
              </td>
            </tr>
             ";
           }
